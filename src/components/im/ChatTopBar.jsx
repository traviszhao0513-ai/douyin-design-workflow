  ...
  if (variant === 'detail') {
    return (
      <header className="cht-nav">
        ...
        <div className="cht-nav__title-stack">
          <span className="cht-nav__title">{title}</span>
          {subtitle && <span className="cht-nav__subtitle">{subtitle}</span>}
        </div>
        <div className="cht-nav__right">
          {showMore ? (
            <button className="cht-nav__action" type="button" aria-label="更多" onClick={onMore}>
              <IcMore width={24} height={24} />
            </button>
          ) : (
            <span className="cht-nav__action" aria-hidden="true" />
          )}
        </div>
    ...
    <header className="cht-nav">
      <div className="cht-nav__left">
        ...
        <div className="cht-nav__contact">
          {showAvatar && <Avatar className="cht-nav__avatar" size="36" src={contactAvatar} alt="" />}
          <div className={contactMeta ? 'cht-nav__name-stack' : 'cht-nav__name-row'}>
            ...
            {contactMeta && <span className="cht-nav__meta">{contactMeta}</span>}
          </div>
        </div>
      </div>
      <div className="cht-nav__right">
        ...
        {showVideo && (
          <button className="cht-nav__action" type="button" aria-label="视频通话">
            <IcVideo width={24} height={24} />
          </button>
        )}
        {showMore && (
          <button className="cht-nav__action" type="button" aria-label="更多" onClick={onMore}>
